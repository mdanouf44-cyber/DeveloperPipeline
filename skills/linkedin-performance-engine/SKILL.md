...
description: Generates 5 LinkedIn posts modeled on Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani)'s OWN top-performing analytics — founder-psychology contrarian, loaded poll, AI-news-with-implications, story carousel, and data-visual-with-hook. Reads the live performance report each run, so the winning patterns update automatically as new analytics come in. This is the data-driven stream — every archetype here earned its place from real impression/engagement numbers, not theory.
allowed-tools: WebFetch, WebSearch, Bash, Read, Write
---

# LinkedIn Performance Engine

You generate the **5 report-driven posts** for the daily Mohammad Anouf Saani batch. Unlike the other engines (which are built on general best-practice), every archetype here is reverse-engineered from **Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani)'s actual LinkedIn analytics**. The brief is simple: do more of exactly what already worked on this specific account.
...
- **AUDIENCE** — adapted from section 6 to target software engineers, developers transitioning to AI/agentic engineering, and technical builders
...
**Every topic must pass the content-doctrine topic filter (Tech Value, Actionability, Educational Depth, Ecosystem Signal) and avoid the DROP list — even the contrarian and the poll.** Aim them directly at technical developers, software engineers, and systems architects.
...
**Why it wins (report data):** Top performer on the account — 413 impressions, the strongest emotional engagement. People resonate deeply with an uncomfortable truth about engineering, building, or tech shifts.

**Doctrine alignment:** Challenge a widely-held belief about development, architectural choices, local execution, AI models, or building. Examples: challenging the idea that "every company needs a massive cloud cluster" when SLMs locally do the job, or "agent frameworks solve all your coordination tasks out of the box." Frame it so developers and builders feel seen.
...
[CTA — "Follow Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani) for more" OR "Repost to help a founder who needs to hear this."]
```
...
**Why it wins (report data):** Highest-impression format on the account. LinkedIn's algorithm rewards interactive content, and emotionally-loaded options drive both votes and reach.

**Formula (report section 3C):** State a hot-button developer debate or architectural dilemma → ask "which one hits closest?" or "what is your biggest bottleneck?" → make all four options emotionally loaded or highly relatable so engineers *want* to vote and argue in comments.

**Doctrine alignment:** Make the dilemma a technical or developer workflow tension (e.g., local model latency, RAG ingestion failures, multi-agent task loops, Docker configs).
...
[CTA — "Follow Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani) for more" or save-prompt.]
```
...
**Why it wins (report data):** Stat visuals build strong brand identity — and they spike when paired with a sharp interpretive hook in the text.

**Build:** Reuse the existing **illustration-formats** skill — do not reinvent.
1. Find one fresh dataset (6-10 data points, 2025-2026, developer tooling, model benchmarks, GitHub trends, or engineering stats). It must NOT be in USED_INFOGRAPHIC_TOPICS and must differ from the main infographic.
...
**Voice check:**
- [ ] Declarative Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani) voice, matches the cadence of the report's WINNING_HOOKS.
