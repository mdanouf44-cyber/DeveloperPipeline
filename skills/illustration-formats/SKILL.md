...
> **Content doctrine gate (`content-doctrine.md` wins).** The dataset must be a technical statistic, developer survey trend, model benchmark (e.g. latency, VRAM footprint, throughput), open-source repository ranking, or engineering benchmark. Out: Vague PR claims or non-technical business metrics. In: local LLM memory overhead, RAG retrieval accuracy, vector search latency, GitHub trends, and Docker/VPS deployment benchmarks. Run the doctrine topic filter on the dataset before building.
...
- Always include `Mohammad Anouf Saani (www.linkedin.com/in/mohammad-anouf-saani)` in the bottom-right footer
- 1080 x 1080 canvas
