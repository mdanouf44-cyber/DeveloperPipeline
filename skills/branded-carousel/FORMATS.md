...
> **Content doctrine gate (`content-doctrine.md` wins).** The carousel topic must be in the technical builder lane: architectural walkthroughs, step-by-step developer guides, local execution benchmarks, and system configurations. Technical lists and guides (e.g. "5 configurations for Ollama local speed," "the 6-step Model Context Protocol data flow," "how to build a two-agent RAG audit loop") are highly encouraged. Run the doctrine topic filter before designing.
